    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900 dark:text-gray-100">
          ...
          {{-- 編集と削除のフォームの下に追加 --}}
          <div class="mt-2">
          @if (auth()->user()->followings->contains($tweet->user_id))
          <form action="{{ route('unfollow', ['user' => $tweet->user_id]) }}" method="POST">
          @csrf
           @method('DELETE')
          <button type="submit" class="text-red-500 hover:text-red-700">Unfollow</button>
         </form>
         @else
         <form action="{{ route('follow', ['user' => $tweet->user_id]) }}" method="POST">
         @csrf
